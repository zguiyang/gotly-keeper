...
## 13. Drizzle Migration Discipline

Database schema, migration files, and database creation flows are Drizzle-managed only.

Rules:

1. Change table, column, index, constraint, and relation definitions in `server/lib/db/schema.ts` first.
2. Generate schema migrations with the project command `pnpm db:generate`; do not hand-write SQL migration files.
3. Apply generated migrations with `pnpm db:migrate` only when the user has approved running a database mutation command.
4. Use `pnpm db:check` as the lightweight static migration check after schema or migration changes.
5. Treat the `out` directory declared in `drizzle.config.ts` as the only Drizzle Kit migration output directory; do not hard-code the directory name in rules, scripts, or reviews.
6. Do not create or keep self-managed database SQL files outside the configured Drizzle Kit output directory.
7. Do not create database-related migration/schema directories by hand, including `server/lib/db/migrations/` or any parallel migration directory.
8. Do not create database SQL files manually; database SQL files must be produced by Drizzle Kit through the project commands.

Data changes:

- use Drizzle ORM application code, typed scripts, or service code paths
- do not store executable SQL migration files outside the Drizzle Kit output directory
- keep one-off data repairs out of the migration system unless they are intentionally modeled through Drizzle-managed migration flow

Verification:

- run `bash .ai-rules/advanced-workflows/guards/check-drizzle-migration-discipline.sh --staged` before commit when schema or migration files are staged
- run `pnpm db:check` after generated migration changes
- run `pnpm db:migrate` only with explicit user approval

---

## 14. Review Checklist (AI and Human)
...
25. Are database schema changes generated through Drizzle Kit instead of hand-written DDL?
