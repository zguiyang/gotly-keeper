...
Database table structure changes are schema-first.

Rules:

1. Change table, column, index, constraint, and relation definitions in `server/lib/db/schema.ts` first.
2. Generate schema migrations with the project command `pnpm db:generate`; do not hand-write schema DDL migrations.
...
5. Treat `drizzle/` as the Drizzle Kit generated migration directory unless `drizzle.config.ts` is intentionally changed in the same task.
6. Do not create schema DDL files under `server/lib/db/migrations/`. That directory may contain legacy or data-only migration helpers, but it must not be extended with new table/column/index DDL.
...
- extension setup or operations that Drizzle Kit cannot represent

Manual SQL constraints:

- state why the SQL cannot be generated by Drizzle Kit
- keep data movement separate from schema DDL
- add rollback or verification notes when data loss or irreversible mutation is possible
