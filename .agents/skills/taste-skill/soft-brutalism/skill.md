---
name: design-taste-soft-brutalism
description: Senior UI/UX Engineer for soft brutalist interfaces combining raw structure with clearer hierarchy and warmth. Uses the full Taste Skill baseline but tunes the output for exposed construction, usable contrast, and softened finish.
---

# High-Agency Soft Brutalism Frontend Skill
...
## 7. STYLE DIRECTION: Soft Brutalism

**When to use this skill:** Use this skill when the user wants brutal structure with more warmth, usability, and a softened material finish.

### Core Identity
- Soft Brutalism is not brutalism-lite; it is raw structure that still cares about readability, warmth, and user respect.
- Borders stay visible, type stays loud, and grids stay exposed, but the page remains usable and contemporary.
- The tension between severity and softness is the point.

### Palette and Material
- Use warm off-whites, strong dark surfaces, visible borders, and one or two material accents like construction orange, oxidized green, or tape yellow.
- Accent should feel earned and structural.
- No candy colors, no fake concrete textures, no gamer neon.

### Typography Character
- Use loud display sans for headlines and calmer, highly readable body type for everything else.
- Mono labels, strong scale contrast, and clear casing help the system feel exposed and intentional.
- The body is the counterweight to the loud display layer.

### Hero Direction
- Use an exposed frame, stacked slab, or offset structural block with one dominant content mass.
- The hero should feel built, not decorated.
- Rounded details can exist inside the system, but the overall silhouette should still feel assertive.

### Navigation Direction
- Use hard but readable rails, underlined active states, or structured segmented rows.
- Navigation should feel part of the system rather than a smooth layer above it.
- Avoid novelty menus and pill spam.

### Layout Tendencies
- Favor exposed grids, thick borders, offset blocks, visible seams, and clear section slabs.
- Compartmentalization should stay readable and not become mess.
- One intentional break from the grid can create drama if the rest of the system is stable.

### Component Vocabulary
- Good fits: exposed-frame cards, stacked slabs, type-driven tags, comparison ledgers, ruled split sections.
- Weak fits: skeuomorphic concrete textures, candy-color neo-brutalism, chaotic collage, luxury haze.

### Media and Imagery
- Use documentary crops, diagrams, bold UI fragments, strong photography, and material closeups.
- Media should reinforce the construction logic.
- Avoid polished lifestyle photography that erases the rawness.

### Motion Profile
- Use quick mechanical transitions, firm hover states, and direct layout shifts with no bounce.
- A small physical press effect can work well.
- Motion should be fast, honest, and never decorative theater.

### Creative Arsenal For This Style
- GSAP Flip layout morph for exposed-frame rearrangements.
- Layout animation list reorder.
- MorphSVG nav indicator only if it stays structurally crisp.

### Style-Specific Avoidances
- candy-color neo-brutalism
- fake concrete textures
- chaos without grid logic
- luxury haze
- plush widgets
- gummy animations
...
- assertive without hostility
- avoid both startup fluff and pseudo-underground noise
- use clear labels and concrete phrasing

### Current Production Priorities
- Combine bold blocks with warm imagery or object crops so the page does not become text-only.
- Add motion with soft-but-assertive transitions: block reveal, image fade, hover offset, and strong press feedback.
- Keep contrast strong but spacing roomy so the hybrid style reads clearly.
- Navigation should show the hybrid identity instead of defaulting to generic minimal nav.
...
- For this style, start with the `Soft Brutalism` section in `../components/style-recipes.md`.
- Strong first picks: GSAP `card-stack`, Aceternity `layout-grid` / `focus-cards`, and React Bits `reflective-card` / `sticker-peel`.
...
- Can you feel the structure under the softness?
- Did warmth improve usability without erasing attitude?
- Is the page sharper than a generic soft SaaS UI?
- Would it still feel distinct without the color palette?
