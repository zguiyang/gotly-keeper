---
name: design-taste-cinematic-product
description: Senior UI/UX Engineer for keynote-style product storytelling and scene-based launch pages. Uses the full Taste Skill baseline but tunes the output for product-as-protagonist composition, chapter flow, and premium motion.
---

# High-Agency Cinematic Product Frontend Skill
...
## 7. STYLE DIRECTION: Cinematic Product

**When to use this skill:** Use this skill when the user wants keynote-style product storytelling, a product launch page, scene-based reveal sequencing, or premium hardware/software presentation.

### Core Identity
- This style is a stage production: every viewport is a scene, every scroll is a chapter cut, and the product is the protagonist.
- It should feel reduced, cinematic, and expensive, not like a generic tech landing page.
- Scene logic matters more than feature-section logic.

### Palette and Material
- Use dark or controlled stage palettes with one product-derived accent and carefully tuned light.
- Off-white text beats pure white; a dark stage should feel tinted and cinematic, not dead black.
- Light should behave like direction, not like generic glow.

### Typography Character
- Use short, declarative headlines and hard hierarchy.
- Treat details like evidence or footnotes that appear later, not like dense copy under the hero.
- Display text can be large but must remain art-directed, not line-broken into mush.

### Hero Direction
- Open with one dominant product moment: device, render, object, UI field, or lit composition.
- The hero should feel like one complete shot, not a text box next to a mockup.
- One strong visual anchor beats three explanatory blocks.

### Navigation Direction
- Use minimal chapter navigation, quiet anchors, or sleek top rails that do not fight the stage.
- Navigation should support sequence.
- Avoid busy app navigation and generic SaaS chrome.

### Layout Tendencies
- Think in scenes, not sections: reveal, detail, comparison, capability, closing shot.
- Use fewer sections but make each one feel authored.
- Big media moments should replace generic feature cards whenever possible.

### Component Vocabulary
- Good fits: reveal stage, chapter strip, comparison frame, exploded detail band, sticky scene, cinematic feature frame.
- Weak fits: FAQ in the middle of the story, testimonial grids, repeated icon cards, dashboard-wall hero.

### Media and Imagery
- Use real product renders, device photography, motion-rich UI captures, material closeups, and chapter stills.
- Media must lead the story.
- Avoid placeholders, stock photos, and default hero screenshots.

### Motion Profile
- Motion can be richer here than in other skills: chapter reveals, spotlight movement, scroll scenes, magnetic CTAs, sequenced transitions.
- GSAP is welcome when the concept earns it.
- Do not create spectacle without clear hierarchy or mix many animation systems carelessly.

### Creative Arsenal For This Style
- Pinned scene with internal scroll animation.
- MotionPath or light-sweep reveal for product edges.
- Interactive device showcase or scroll-driven image sequence when conceptually justified.

### Style-Specific Avoidances
- Apple parody without a point of view
- random parallax everywhere
- generic dark gradient tech page
- feature-card rows as main structure
- weak product imagery
- game-trailer motion excess

### Copy Tone
- short, declarative, product-first
- scene-setting rather than feature dumping
- supporting copy should be precise and sparse
- avoid launch-page fluff

### Current Production Priorities
- Product or campaign imagery is mandatory: use a hero still, object render, dramatic crop, or cinematic background image by default.
- Proof stats like `Active Nodes` or `Species Tracked` should feel more sculptural, animated, and visually staged rather than plain utility cards.
- Add tasteful motion by default: hero fades, staged reveals, media movement, hover light shifts, and premium button feedback.
- Navigation should feel more like a product launch or campaign rail than a generic SaaS top bar.
...
- For this style, start with the `Cinematic Product` section in `../components/style-recipes.md`.
- Strong first picks: GSAP `image-sequence` / `flip-gallery-modal`, Aceternity `hero-parallax` / `apple-cards-carousel`, and React Bits `split-text` / `glare-hover`.
...
- Would the product still feel like the star if motion disappeared?
- Does each section feel like a new scene?
- Is the spectacle controlled?
- Did you avoid default launch-page clich̩s?
