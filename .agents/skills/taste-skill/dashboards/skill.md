---
name: design-taste-dashboards
description: Senior UI/UX Engineer for polished operator dashboards, admin surfaces, control panels, and monitoring interfaces. Uses the full Taste Skill baseline but tunes the output for task-first hierarchy, actionable density, and calm operational clarity.
---

# High-Agency Dashboards Frontend Skill
...
## 7. STYLE DIRECTION: Dashboards

**When to use this skill:** Use this skill for dashboards, control panels, admin surfaces, monitoring views, operator workspaces, and system-management screens.

### Core Identity
- A dashboard is a tool surface for noticing state and acting fast.
- The page should feel like a workspace already in use, not a homepage with stats attached.
- Operational clarity matters more than branding theater.

### Palette and Material
- Use calm neutrals, controlled surface ladders, and sparse semantic accents.
- No neon glows, no colorful ambient halos, no attention-seeking backgrounds by default.
- Numbers and states should stay readable in grayscale logic as well as color logic.

### Typography Character
- Use disciplined sans pairings with mono for data, paths, queues, timestamps, IDs, and technical metadata.
- Keep the heading subordinate to the work surface.
- Labels and status text should be short, precise, and operational.

### Hero Direction
- The first screen must answer what the system is doing right now.
- Open with useful UI: overview strip, command bar, queue, board, live slab, or operator split.
- The page must start with work, not with a marketing headline or intro paragraph.

### Navigation Direction
- Use segmented workspace navigation, command rails, sidebar anchors, or compact utility-led top bars.
- Separate workspace orientation from utilities when needed.
- Primary nav should never depend on sideways text or decorative pills.

### Layout Tendencies
- Use overview strip, primary operational slab, secondary inspection slab, and entities or logs in clear vertical sequence.
- Prefer bands, rails, tables, split panes, maps, timeline strips, and dense slabs over KPI-card rows.
- Dashboard metrics should feel like one instrument surface, not detached pricing cards.

### Component Vocabulary
- Good fits: status band, queue plus detail panel, command input, map plus event rail, spec matrix, table with drilldown.
- Weak fits: logo strips, testimonial blocks, giant hero copy, empty luxury whitespace, decorative charts.

### Media and Imagery
- Charts, queues, maps, traces, workflow previews, and tables are the hero media here.
- Photography is allowed only when it belongs to the workflow or the context.
- Every graph and every metric needs a reason.

### Motion Profile
- Use motion for sorting, filtering, live-state change, drawer movement, queue arrival, status emphasis, and layout continuity.
- Avoid theatrical scroll reveals, generic fade-up on every block, and purposeless perpetual loops.
- Motion should explain state rather than decorate it.

### Creative Arsenal For This Style
- Typewriter command palette.
- Morphing SVG nav indicator or gooey tab indicator only if the product tone allows it.
- Contextual UI focus-mode slab with staggered highlight.

### Style-Specific Avoidances
- marketing hero copy
- three stat cards as the whole page
- pill spam
- halo glows
- luxury whitespace with no function
- decorative charts with no action path

### Copy Tone
- task-first
- clear and operational
- short labels over prose
- decisive status language
- avoid vague mission statements

### Current Production Priorities
- Use charts, diagrams, process visuals, maps, timelines, and system graphics by default instead of relying only on cards and tables.
- Avoid internal card scrollbars. Expand the panel, paginate it, or move dense streams into dedicated surfaces if scrolling is necessary.
- Add subtle live-state motion: count-up reveals, chart draw-ins, row highlights, filter transitions, and hover states on controls.
- Toolbar and navigation areas should feel purposeful and slightly more designed than a stock dashboard header.
...
- For this style, start with the `Dashboards` section in `../components/style-recipes.md`.
- Strong first picks: GSAP layout/scroll demos, 21st progress + card primitives, React Bits `count-up`, and diagram-heavy sections instead of more metric cards.
...
- Can the user tell what matters in five seconds?
- Is the first screen usable instead of promotional?
- Did you avoid turning every module into the same card shell?
- Does motion clarify state?
